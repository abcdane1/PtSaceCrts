#sace glm 
saceglm<-function(data,trt="A",surv="S",out="Y",clustid="Id",indv="X",crobust=T,varpen=F,set1=T,set2=T,conf=.95,boot=F,iters=200){
  ...
  if(boot==F){
    ...
    for(i in 1:nrow(dffullint)){
      ...
      if(set2==T){
        ...
        ##phi2noij<-Yi*Ai*Si*valstnoi/valsonoi-exp1j*Ai*Si*valstnoi/valsonoi
        ##phi3noij<-Yi*(1-Ai)*Si-exp0j*(1-Ai)*Si
        #A21noij<-dfcovfullopi*Yi*Ai*Si*valstnoi*(1-valstnoi)/valsonoi-dfcovfulli*Yi*Ai*Si*valstnoi*(1-valsonoi)/valsonoi)-exp1j*apply(dfcovop*Ai*Si*valstnoi*(1-valstnoi)/valsonoi-dfcov*Ai*Si*valstnoi*(1-valsonoi)/valsonoi,2,sum)
        #A31noij<-rep(0,ncov)
