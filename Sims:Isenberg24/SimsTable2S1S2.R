#simulations, parallelized for mac or linux 
#analytic variance only, boot must be set to FALSE 
#creates values as in Tables 2, S1, S2
...
#names for columns of simulation output 
...
newwrapparamest_sim<-function(nc,a,b,alphamon,alphasurv,iccy,iccs,crobust,boot,iters,nsim,ncores){
  ...
  newparamest_sim<-function(nc,a,b,alphamon,alphasurv,iccy,iccs,crobust,boot,iters,nsim){
    newdata_sim<-function(nc,a,b,alphamon,alphasurv,iccy,iccs,nsim){
      ...
      #bi<-gam*rnorm(nc,mean=0,sd=sqrt(sig2y)), for independent
      for(i in 1:nc){
  ...
  lw<-l[1:15,] #performance measures for SACE glmm  
  convw<-l[16,] #convergence 
  lwo<-l[17:31,] #performance measures for SACE glm
  convwo<-l[32,] #convergence (set to 0, so always)
  resultswomcw<-apply(lw,1,mean,na.rm=T) #measures that need to be averaged
  convwtot<-sum(convw,na.rm=T)
  nsimempw<-nsim-convwtot #remove failed convergence
  #mc est of variance 
  mcestvarset1w<-1/((nsimempw-1))*sum((lw[2,]-resultswomcw[2])^2,na.rm=T) 
  mcestvarset2w<-1/((nsimempw-1))*sum((lw[3,]-resultswomcw[3])^2,na.rm=T)
  resultsw<-c(resultswomcw[1:5],mcestvarset1w,mcestvarset2w,resultswomcw[6:15],convwtot) #output for glmm
  
  #repeat above for glm
...
#bscen<-c(40,150)
bscen<-c(50) #max clustersize
alphamonscen<-c(log(1.25),0,-log(1.25),log(5)) #treatment effect on survival
#alphamonscen<-log(1.25)
alphasurvscen<-0 #remove effect of other survival
iccyscen<-.1 #ICC for non-mortal outcome
#iccscen cant be identically 0
iccsscen<-c(.3,.15,.1, 0.05) #ICC for survival status
#iccsscen<-c(.05)
...
for(i in 1:(nrow(finscenariomat)/2)){
  ...
  if(i %% 6 == 0){ 
    filename<-paste("new5sacescendf",i,".csv",sep="")
